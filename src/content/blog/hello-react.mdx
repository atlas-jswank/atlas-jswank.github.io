...
You may have noticed this syntax for createing react components doesnt look like valid javascript. In javascript you cannot just return html from a javascript function. This is used in react a special syntax called JSX.
...
Utilimately this javascript version is what the browser will execute. Just know that browsers do not understand JSX so it will always need to be transpiled to plain javascript. That is why we use tools such as Vite. 

You will find that writing your react code in JSX makes it much easier to read.
